...
// canReuseNode checks whether a subtree from the old tree can be reused
// by the parser. Returns true if the subtree can be pushed onto the parse
// stack without re-parsing.
func canReuseNode(s Subtree, arena *SubtreeArena, tableEntry *TableEntry, currentLexMode LexMode,
	firstLeafLexMode LexMode) bool {

	// 1. has_changes flag — the subtree was modified by an edit.
	if HasChanges(s, arena) {
		return false
	}

	// 2. Error nodes cannot be reused.
	sym := GetSymbol(s, arena)
	if sym == SymbolError || sym == SymbolErrorRepeat {
		return false
	}

	// 3. Missing nodes (inserted by error recovery) cannot be reused.
	if IsMissing(s, arena) {
		return false
	}

	// 4. Fragile nodes (from GLR ambiguity) cannot be reused.
	if IsFragileLeft(s, arena) || IsFragileRight(s, arena) {
		return false
	}

	// 5. Parse table reusability check: the table entry header must have
	// Reusable = true for this symbol in the current state.
	if !tableEntry.Reusable {
		return false
	}

	// 6. Lex mode compatibility: the subtree's first leaf must have been
	// lexed in a compatible mode.
	firstLeaf := GetFirstLeaf(s, arena)
	if firstLeaf.ParseState != 0 {
		// Look up the lex mode for the state where this leaf was originally parsed.
		origLexMode := firstLeafLexMode
