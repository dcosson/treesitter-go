...
type (
	...
	Point         = core.Point
	Range         = core.Range
	InputEdit     = core.InputEdit
	FieldMapEntry = core.FieldMapEntry

	Language     = language.Language
	Subtree      = st.Subtree
	SubtreeArena = st.SubtreeArena
	SubtreeID    = st.SubtreeID
)

// Re-import subtree functions used in this package.
var (
...
// GetSubtree returns the underlying Subtree value for this node.
// Used by tests and internal packages that need low-level subtree access.
func (n Node) GetSubtree() Subtree {
	return n.subtree
}
...
// Exported for use by tree_cursor in the same package and query_cursor in the root package.
