...
func TestCostConstants(t *testing.T) {
	if MaxCostDifference != 18*ErrorCostPerSkippedTree {
		t.Fatalf("MaxCostDifference = %d, want %d", MaxCostDifference, 18*ErrorCostPerSkippedTree)
	}
	if MaxVersionCount != 6 {
		t.Fatalf("MaxVersionCount = %d, want 6", MaxVersionCount)
	}
	if MaxSummaryDepth != 16 {
		t.Fatalf("MaxSummaryDepth = %d, want 16", MaxSummaryDepth)
	}
}
